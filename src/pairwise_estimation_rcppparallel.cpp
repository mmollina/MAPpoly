          ...
          for(int i=0; i < count_matrix_number[(id-1)]; i++) //MODIFIED
            {
              ...
              if (min_lod > res2){         // Checking if current phase has LOD higher than previous highest phase
                out(k,0) = i;     // Saving best phase position
                out(k,1) = x;              // Store rf
                ...
                for(int i = 0; i < n_ind; i++)
                  temp += log10(Tr[gen_1[i] + (gen_2[i] * (m+2))]);
                out(k,2) = std::abs(res2+temp); // Saving LOD-rf for best phase
                out(k,3) = std::abs(min_lod - res2); // Account difference to second phase LOD
                min_lod = res2;            // Update min lod control
              }
              else if (std::abs(min_lod - res2) < out(k,3)){
                out(k,3) = std::abs(min_lod - res2); // Account difference to second phase LOD
              }
            }
        }
                                                 ...
                                                 SEXP threads_R)
{
  ...
  NumericMatrix out(mrk_pairs.ncol(), 4);
  int m = as<int>(m_R);
  double tol = as<double>(tol_R);
  int threads = as<int>(threads_R);
  int n_ind = geno.ncol();

  rf_two_point_parallel RF_two_point_parallel(mrk_pairs, geno, dP, dQ, count_vector, count_matrix_rownames, count_matrix_number, count_matrix_pos, count_matrix_length, m, tol, n_ind, out);
