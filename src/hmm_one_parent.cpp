...
RcppExport SEXP est_hmm_map_one_parent_highprec(SEXP ploidyR,
                                                SEXP n_mrkR,
                                                SEXP n_indR,
                                                SEXP haploR,
                                                SEXP emitR,
                                                ...
                                                SEXP ret_H0R)
{
  ...
  //Initializing some variables
  int k, k1,  maxit = 1000, flag=0;
  double loglike=0.0, nr=0.0, temp=0.0;
  long double s;
  std::vector<double> rf_cur(rf.size());
  std::vector<double> term(n_ind);
  std::fill(term.begin(), term.end(), 0.0);
  ...
  for(int ind=0; ind < n_ind; ind++)
  {
    for(int i=0; i < n_mrk; i++)
    {
      std::vector<long double> temp3(v[i][ind].size());
      alpha[ind].push_back(temp3);
      beta[ind].push_back(temp3);
    }
  }
  ...
  for(int it=0; it<maxit; it++)
  {
    ...
    for(int ind=0; ind < n_ind; ind++)
    {
      ...
      for(k=1,k1=n_mrk-2; k < n_mrk; k++, k1--)
      {
        std::vector<long double> temp4 (v[k][ind].size());
        temp4 = forward_emit_one_parent_highprec(m, alpha[ind][k-1], v[k-1][ind], v[k][ind], e[k][ind], T[k-1]);
        for(int j=0; (unsigned)j < temp4.size(); j++)
        {
          alpha[ind][k][j]=temp4[j];
        }
        std::vector<long double> temp5 (v[k1][ind].size());
        temp5=backward_emit_one_parent_highprec(m, beta[ind][k1+1], v[k1][ind], v[k1+1][ind], e[k1+1][ind], T[k1]);
        for(int j=0; (unsigned)j < temp5.size(); j++)
        {
          beta[ind][k1][j]=temp5[j];
        }
