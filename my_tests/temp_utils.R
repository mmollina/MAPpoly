...
add_md_markers <- function(input.map, 
                           ...
                           thresh = 500, 
                           extend.tail = 50,
                           method = c("ols", "hmm", "wMDS_to_1D_pc"), 
                           verbose = T){
  method <- match.arg(method)
  ...
  if(method == "ols"){
    map.p1.p2.final <- reest_rf(map.p1.p2.final, method = "ols", input.mat = full.mat)
  } else if(method == "hmm"){
                        ...
                        gap.threshold = 4)
{
  ...
  while(length(mrk.to.include) > 0 | i >= length(la)){
    ...
    if(a$info$n.mrk < cur.res$map$info$n.mrk){
      if(method == "ols"){
        cur.res$map <- reest_rf(a, method = "ols", input.mat = full.mat)
      } else if(method == "hmm"){
        cur.res$map <- reest_rf(a, method = "hmm", verbose = FALSE)
      } else if(method == "wMDS_to_1D_pc"){
