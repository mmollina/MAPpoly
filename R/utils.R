...
plot_compare_haplotypes <- function(ploidy, hom.allele.p1, hom.allele.q1, hom.allele.p2 = NULL, hom.allele.q2 = NULL) {
  nmrk <- names(hom.allele.p1)
  oldpar <- par(mar = c(5.1, 4.1, 4.1, 2.1))
  on.exit(par(oldpar))
  u1 <- ph_list_to_matrix(hom.allele.p1, ploidy)
  u2 <- ph_list_to_matrix(hom.allele.q1, ploidy)
  if (!is.null(hom.allele.p2)) {
    u3 <- ph_list_to_matrix(hom.allele.p2, ploidy)
    o3 <- apply(cor(u1, u3), 1, which.max)
    hom.allele.p2 <- ph_matrix_to_list(u3[, o3])
  }
  if (!is.null(hom.allele.q2)) {
    u4 <- ph_list_to_matrix(hom.allele.q2, ploidy)
    o4 <- apply(cor(u2, u4), 1, which.max)
    hom.allele.q2 <- ph_matrix_to_list(u4[, o4])
  }
  ...
  #text(x = pos.p, y = rep(-(ploidy+1), length(pos.p)), labels = nmrk, cex = .5)
  pos.q <- pos.p + 12
  for (i in 1:n.mrk) {
  ...
  #text(x = pos.q, y = rep(-(ploidy+1) , length(pos.q)), labels = nmrk, cex = .5)
