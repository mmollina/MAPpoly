...
#'     
#' @export
plot_mrk_info<-function(input.data, mrk.name)
  {
  ...
  if(!mrk.name%in%input.data$mrk.names)
    stop(deparse(substitute(mrk.name)), " is not presnet in ", deparse(substitute(input.data)), "dataset")
  ## Parents dosage
  dp<-input.data$dosage.p[input.data$mrk.names==mrk.name]
  dq<-input.data$dosage.q[input.data$mrk.names==mrk.name]
  
  ## if no probabilities
  if(length(input.data) == 13){
    layout(matrix(c(1,2), ncol = 2), widths = c(1, 2))
    ## Genotype frequencies
    x<-input.data$geno.dose[mrk.name, ]
    ...
    mtext(side = 3, text = mrk.name, adj = 0, cex = 1.2, font = 3)
    text(x = 0, y = 90 , labels = paste0("marker #: ", which(input.data$mrk.names==mrk.name)), adj = 0)
    ## parents dosage
    ...
    text(x = 0, y = 50 , labels = paste0("p-value: ", round(pval, 4)), adj = 0)
    ##genomic position and sequence
    text(x = 0, y = 40 , labels = paste0("sequence: ", input.data$sequence[input.data$mrk.names==mrk.name]), adj = 0)
    text(x = 0, y = 30 , labels = paste0("seq. position: ", input.data$sequence.pos[input.data$mrk.names==mrk.name]), adj = 0)
    par(op)
    barplot(x, col = c(gg_color_hue(input.data$m + 1)[1:(length(x)-1)], "#404040"))
  } else if(length(input.data) == 14){ ## if genotype probabilities are available
