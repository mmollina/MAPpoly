...
#' @examples
#'   \donttest{
#'  ## Example with first 3 chromosomes of tetraploid potato
#'  solcap.p<-vector("list", 3)
...
#'              solcap.p[i]))
#'  names(w) <- apply(expand.grid(c("dose", "prior"), paste0("LG_", 1:3), 
#'                              stringsAsFactors = FALSE)[,2:1], 1, paste, 
#'                  collapse = "_")
#'  op <- par(cex.axis = .7)
#'  plot_map_list(w, horiz = FALSE, col = rep(gg_color_hue(2), 3))
#'  par(op)
