...
<img src="https://raw.githubusercontent.com/mmollina/MAPpoly_vignettes/master/figures/filter_ind.png" width="388"/>
...
```{r, twopt_hide_1, echo=FALSE, results='hide', }
ncores <-2
in.file <- "https://github.com/mmollina/SCRI/raw/main/docs/tetra/tpt.rda"
tpt_file <- tempfile()
download.file(in.file,tpt_file)
load(tpt_file)
#save(tpt, file = "~/tpt.rda", compress = "xz", compression_level = 9)
#tpt <- est_pairwise_rf2(seq.init, ncpus = ncores)
```

```{r, twopt_hide_2, echo=FALSE}
m <- rf_list_to_matrix(tpt) ## converts rec. frac. list into a matrix 
sgo <- make_seq_mappoly(go) ## creates a sequence of markers in the genome order
plot(m, ord = sgo, fact = 5) ## plots a rec. frac. matrix using the genome order, averaging neighbor cells in a 5 x 5 grid 
```
...
```{r, map_lg1_load, results = FALSE, echo=FALSE}
in.file <- "https://github.com/mmollina/SCRI/raw/main/docs/tetra/tpt1.rda"
tpt1_file <- tempfile()
download.file(in.file, tpt1_file)
load(tpt1_file)
in.file <- "https://github.com/mmollina/SCRI/raw/main/docs/tetra/lg1_map.rda"
lg1_file <- tempfile()
download.file(in.file, lg1_file)
load(lg1_file)
#save(lg1.map, file = "~/lg1_map.rda", compress = "xz", compression_level = 9)
#tpt <- est_pairwise_rf2(seq.init, ncpus = ncores)
```
